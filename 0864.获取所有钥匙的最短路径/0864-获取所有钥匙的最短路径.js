        ...
        for (let j = 0; j < n; ++j) {
            if (grid[i][j] === '@') {
                ...
                if (!keyToIndex.has(grid[i][j])) {
                    ...
                    keyToIndex.set(grid[i][j], idx);  // 第几把锁
                }
            }
        }
    }
        ...
        for (let i = 0; i < 4; ++i) {
            ...
            if (nx >= 0 && nx < m && ny >= 0 && ny < n && grid[nx][ny] !== '#') {
                if (grid[nx][ny] === '.' || grid[nx][ny] === '@') {
                    if (dist[nx][ny][mask] === -1) { // 尚未被访问过
                        dist[nx][ny][mask] = dist[x][y][mask] + 1;
                        queue.push([nx, ny, mask]);
                    }
                } else if ('a' <= grid[nx][ny] && grid[nx][ny] <= 'z') {
                    const idx = keyToIndex.get(grid[nx][ny]);
                    if (dist[nx][ny][mask | (1 << idx)] === -1) { // 尚未被访问
                        dist[nx][ny][mask | (1 << idx)] = dist[x][y][mask] + 1;
                        if ((mask | (1 << idx)) === (1 << keyToIndex.size) - 1) { // 全部的钥匙被拿到
                            return dist[nx][ny][mask | (1 << idx)];
                        }
                        queue.push([nx, ny, mask | (1 << idx)]);
                    }
                } else {
                    const idx = keyToIndex.get(grid[nx][ny].toLowerCase()); // 碰到了锁
                    if ((mask & (1 << idx)) !== 0 && dist[nx][ny][mask] === -1) { // 有钥匙且未被访问
                        dist[nx][ny][mask] = dist[x][y][mask] + 1;
                        queue.push([nx, ny, mask]);
                    }
                }
